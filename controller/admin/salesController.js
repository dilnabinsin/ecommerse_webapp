...
const loadSalesPage = async (req, res) => {
  try {
      ...
      res.status(HTTP_STATUS.OK).render('sales-report', { salesData });
  } catch (error) {
      console.error('Error in loadSalesPage:', error);
      return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).render('admin/pageerror', { 
...
const createSaleRecord = async (order) => {
  try {
    ...
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ message: 'Failed to generate Excel file' });
    throw error;
  }
};
